...
suite("Category API tests", () => {
  ...
  test("create category", async () => {
    const returnedCategory = await placemarkService.createCategory(arunas);
    assert.isNotNull(returnedCategory);
    assertSubset(arunas, returnedCategory);
  });

  test("delete a category", async () => {
    const category = await placemarkService.createCategory(arunas);
